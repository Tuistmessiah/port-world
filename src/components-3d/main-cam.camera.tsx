...
export function MainCam() {
    ...
    function goToCareerSections() {
        switch (page.section) {
            ...
            case 'elanza':
                ...
                break;
            case 'thepeoplegroup':
                moveToTarget(rootState.camera, new THREE.Vector3(...camPoints(device).career.thepeoplegroup.position), new THREE.Vector3(...camPoints(device).career.thepeoplegroup.lookAt), 2000);
                break;
            default:
    ...
    // // TODO: 1) Pass a reference of OrbitControls to Recoil and use it in main-cam OR just add OrbitControls and its ref inside main-cam.
    // // TODO: 2) Follow ChatGPT's suggestion and see what happens (probably need to be able to set a target point for orbit controls and a cam position)
    // TODO: Just have a button to go to free control mode and when a button is clicked, leave free control mode again
    // console.log({ orbitControls: orbitControlsRef as any });
    // if (orbitControlsRef.current) orbitControlsRef.current.target.set(40, 40, 40);
    // (orbitControls.current as any).target.set(50, 50, 50);
